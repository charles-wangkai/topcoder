...
public class ContestScore {
  public String[] sortResults(String[] data) {
            ...
            .map(
                d -> {
                  ...
                  for (int i = 0; i < scores.length; ++i) {
                    scores[i] = Integer.parseInt(parts[i + 1].replace(".", ""));
                  }
    ...
    for (Group group : groups) {
      group.totalRank =
          IntStream.range(0, scoreNum)
              .map(
                  i ->
                      1
                          + (int)
                              Arrays.stream(groups)
                                  .filter(g -> g.scores[i] > group.scores[i])
                                  .count())
              .sum();
    ...
    return Arrays.stream(groups)
        .sorted(
            Comparator.comparing((Group group) -> group.totalRank)
                .thenComparing(Comparator.comparing((Group group) -> group.totalScore).reversed())
                .thenComparing(group -> group.name))
        .map(
            group ->
                String.format(
                    "%s %d %d.%d",
                    group.name, group.totalRank, group.totalScore / 10, group.totalScore % 10))
        .toArray(String[]::new);
  }
