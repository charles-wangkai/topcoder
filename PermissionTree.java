  ...
  public int[] findHome(String[] folders, String[] users) {
    ...
    for (int i = 0; i < folders.length; ++i) {
      ...
      for (String user : parts[1].split(",")) {
        userToFolders.putIfAbsent(user, new HashSet<>());
        userToFolders.get(user).add(i);
      }
    }

    return Arrays.stream(users)
        .mapToInt(user -> computeHome(parents, userToFolders, user))
        .toArray();
  }

  int computeHome(int[] parents, Map<String, Set<Integer>> userToFolders, String user) {
    int result = -1;
    for (int folder : userToFolders.getOrDefault(user, Set.of())) {
      result = (result == -1) ? folder : findLCA(parents, result, folder);
    }
