...
public class PaternityTest {
	public int[] possibleFathers(String child, String mother, String[] men) {
		...
		return IntStream.range(0, men.length).filter(i -> {
			for (int code = 0; code < (1 << length); ++code) {
				if (Integer.bitCount(code) * 2 == length) {
					boolean[] chosen = decode(length, code);
					if (IntStream.range(0, length).allMatch(j -> (chosen[j] && child.charAt(j) == mother.charAt(j))
							|| (!chosen[j] && child.charAt(j) == men[i].charAt(j)))) {
						return true;
					}
				}
			}
	...
	boolean[] decode(int length, int code) {
	...
	public static void main(String[] args) {
		...
		p2 = new String[] { "WNELPYCHXWXPCMNKDDXD", "WFEEPYCHFWDNPMKKALIW", "WSEFPYCHEWEFGMPKIQCK",
