...
public class InstantRunoff {
	public String outcome(String candidates, String[] ballots) {
		while (!candidates.isEmpty()) {
			Map<Character, Integer> candidateToCount = candidates.chars().boxed()
					.collect(Collectors.toMap(ch -> (char) ch.intValue(), ch -> 0));
			...
			int maxCount = candidateToCount.values().stream().mapToInt(x -> x).max().getAsInt();
			if (maxCount * 2 > ballots.length) {
				return String.valueOf(candidateToCount.keySet().stream()
						.filter(candidate -> candidateToCount.get(candidate) == maxCount).findAny().get());
			}

			int minCount = candidateToCount.values().stream().mapToInt(x -> x).min().getAsInt();
			String pattern = String.format("[%s]",
					candidateToCount.keySet().stream().filter(candidate -> candidateToCount.get(candidate) == minCount)
