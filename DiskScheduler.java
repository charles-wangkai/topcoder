...
public class DiskScheduler {
  ...
  public int optimize(int start, int[] sectors) {
    int[] sorted =
        IntStream.concat(
                Arrays.stream(sectors).filter(s -> s > start).sorted(),
                Arrays.stream(sectors).filter(s -> s < start).sorted())
            .toArray();
    ...
    for (int i = sorted.length - 1; i >= -1; --i) {
      int leftDistance = Math.floorMod(start - prev, SECTOR_NUM);
      int rightDistance = Math.floorMod(((i == -1) ? start : sorted[i]) - start, SECTOR_NUM);

      result =
          Math.min(result, leftDistance + rightDistance + Math.min(leftDistance, rightDistance));
