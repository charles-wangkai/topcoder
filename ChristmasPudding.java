...
public class ChristmasPudding {
  public long eat(
      int N, long Vall, long Vsweet, int[] Vprefix, int[] Tprefix, int[] Sprefix, int seed) {
    ...
    int[] sweetIndices =
        IntStream.range(0, N)
            .filter(i -> S[i] == 1)
            .boxed()
            .sorted(Comparator.comparing((Integer i) -> T[i]).reversed())
    ...
    int sweetIndex = 0;
    while (Vsweet != 0 && sweetIndex != sweetIndices.length) {
      int index = sweetIndices[sweetIndex];
    ...
    int savoryIndex = 0;
    while (Vall != 0
        && (sweetIndex != sweetIndices.length || savoryIndex != savoryIndices.length)) {
      int index;
      if (savoryIndex == savoryIndices.length
          || (sweetIndex != sweetIndices.length
              && T[sweetIndices[sweetIndex]] >= T[savoryIndices[savoryIndex]])) {
        index = sweetIndices[sweetIndex];
        ++sweetIndex;
      } else {
        index = savoryIndices[savoryIndex];
        ++savoryIndex;
      }
  ...
  int nextState(int state) {
    return (int) Math.floorMod(state * 1_103_515_245L + 12345, 1L << 31);
  }
