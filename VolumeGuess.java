...
public class VolumeGuess {
  public int correctVolume(String[] queries, int numberOfBoxes, int ithBox) {
    ...
    int maxVolume = Arrays.stream(queryArray).mapToInt(query -> query.volume).max().getAsInt();
    for (int i = 0; ; ++i) {
    ...
    while (true) {
      ...
      for (Query query : queryArray) {
        if (volumes[query.box1] == -1
            && volumes[query.box2] != -1
            && volumes[query.box2] != query.volume) {
          volumes[query.box1] = query.volume;
          modified = true;
        } else if (volumes[query.box1] != -1
            && volumes[query.box2] == -1
            && volumes[query.box1] != query.volume) {
          volumes[query.box2] = query.volume;
          modified = true;
