public class CardboardBoxes {
  public long count(long S) {
    ...
    for (int i = 2; (long) i * i <= product; ++i) {
      if (product % i == 0) {
        long another = product / i;

        result += computeWayNum(i, another);
        if (another != i) {
          result += computeWayNum(another, i);
        }
      }
    }
  ...
  long computeWayNum(long sumAB, long sumAH) {
    return Math.min(sumAB / 2, sumAH - 1);
  }
