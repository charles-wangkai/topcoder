...
public class PowerOutage {
  ...
  public int estimateTimeOut(int[] fromJunction, int[] toJunction, int[] ductLength) {
    ...
    List<Integer>[] edgeLists = new List[MAX_SIZE];
    for (int i = 0; i < edgeLists.length; ++i) {
      edgeLists[i] = new ArrayList<>();
    }
    for (int i = 0; i < fromJunction.length; ++i) {
      edgeLists[fromJunction[i]].add(i);
    }

    maxPathLength = -1;

    return search(toJunction, ductLength, edgeLists, 0, 0) * 2 - maxPathLength;
  }

  int search(
      int[] toJunction, int[] ductLength, List<Integer>[] edgeLists, int pathLength, int node) {
