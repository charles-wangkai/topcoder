...
public class ElevatorButtons {
	public int[] nextStops(int currentFloor, int currentDirection, int[] buttonsPressed) {
		...
		IntStream down = Arrays.stream(buttonsPressed).filter(x -> x < currentFloor).distinct().boxed()
				.sorted(Collections.reverseOrder()).mapToInt(x -> x);

		return ((currentDirection == 1) ? IntStream.concat(up, down) : IntStream.concat(down, up)).toArray();
	}
