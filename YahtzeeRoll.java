...
public class YahtzeeRoll {
  public double bestChoice(int[] dice) {
    return IntStream.range(0, 1 << dice.length)
        .mapToDouble(mask -> search(dice, mask, 0, 1))
        .max()
        .getAsDouble();
  }

  double search(int[] dice, int mask, int index, double prob) {
    if (index == dice.length) {
      return prob * computeScore(dice);
    }

    if (((mask >> index) & 1) == 0) {
      return search(dice, mask, index + 1, prob);
    }
    ...
    for (int i = 1; i <= 6; ++i) {
      dice[index] = i;
      result += search(dice, mask, index + 1, prob / 6);
    }
  ...
  int computeScore(int[] dice) {
    ...
    if (distinctSorted.length >= 4
        && ((distinctSorted[0] + 1 == distinctSorted[1]
                && distinctSorted[1] + 1 == distinctSorted[2]
                && distinctSorted[2] + 1 == distinctSorted[3])
            || (distinctSorted.length == 5
                && distinctSorted[1] + 1 == distinctSorted[2]
                && distinctSorted[2] + 1 == distinctSorted[3]
                && distinctSorted[3] + 1 == distinctSorted[4]))) {
      return 30;
    }
