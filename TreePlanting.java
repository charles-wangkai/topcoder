public class TreePlanting {
	public long countArrangements(int total, int fancy) {
		long[][] wayNums = new long[fancy + 1][2];
		wayNums[0][0] = 1;
		for (int i = 0; i < total; ++i) {
			long[][] nextWayNums = new long[fancy + 1][2];
			for (int j = 0; j <= fancy; ++j) {
				nextWayNums[j][0] = wayNums[j][0] + wayNums[j][1];

				if (j != 0) {
					nextWayNums[j][1] = wayNums[j - 1][0];
				}
			}
		...
		return wayNums[fancy][0] + wayNums[fancy][1];
	}
