...
public class ParenReduction {
  static final Map<Character, Integer> OPERATOR_TO_PRECEDENCE =
      Map.ofEntries(entry('^', 1), entry('*', 2), entry('/', 2), entry('+', 3), entry('-', 3));

  public String pretty(String expr) {
    return format(expr).formatted;
  }

  Outcome format(String expr) {
    if (expr.length() == 1) {
      return new Outcome(0, expr);
    }
    ...
    int precedence = OPERATOR_TO_PRECEDENCE.get(operator);

    Outcome leftResult = format(expr.substring(0, index));
    Outcome rightResult = format(expr.substring(index + 1));

    String formatted =
        String.format(
            "%s%c%s",
            (leftResult.precedence > precedence
                    || (leftResult.precedence == precedence && operator == '^'))
    ...
    return new Outcome(precedence, formatted);
  }
  ...
  class Outcome {
    ...
    Outcome(int precedence, String formatted) {
