...
public class PolynomialMultiplier {
	public String product(String f1, String f2) {
		...
		return expToCoef.keySet().stream().sorted(Collections.reverseOrder()).map(exp -> {
			...
			if (exp == 1) {
				return String.format("%d*x", coef);
			} else {
				return String.format("%d*x^%d", coef, exp);
			}
