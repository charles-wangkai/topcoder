...
public class KRectangleIntersection {
    ...
    for (int cxl : xl) {
      for (int cxh : xh) {
        if (cxl < cxh) {
          Range[] ranges =
              IntStream.range(0, n)
                  .filter(i -> xl[i] <= cxl && xh[i] >= cxh)
                  .mapToObj(i -> new Range(yl[i], yh[i]))
                  .sorted(Comparator.comparing(range -> range.left))
                  .toArray(Range[]::new);
          ...
          for (Range range : ranges) {
            ...
            if (rights.size() == k) {
              result =
                  Math.max(result, (long) (cxh - cxl) * Math.max(0, rights.peek() - range.left));
            }
          }
        }
      }
  ...
  private static boolean KawigiEdit_RunTest(
      int testNum, int[] p0, int[] p1, int[] p2, int[] p3, int p4, boolean hasAnswer, long p5) {
