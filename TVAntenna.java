...
public class TVAntenna {
	public double minRadius(int[] x, int[] y) {
		...
		return IntStream.rangeClosed(minX, maxX).boxed().flatMapToDouble(cx -> IntStream.rangeClosed(minY, maxY)
				.mapToDouble(cy -> IntStream.range(0, x.length).mapToDouble(i -> {
					int dx = x[i] - cx;
					int dy = y[i] - cy;
