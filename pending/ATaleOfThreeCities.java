...
public class ATaleOfThreeCities {
	...
	double computeMinDistance(int[] x1, int[] y1, int[] x2, int[] y2) {
		return IntStream.range(0, x1.length).mapToDouble(i -> IntStream.range(0, x2.length)
				.mapToDouble(j -> Math.sqrt((x1[i] - x2[j]) * (x1[i] - x2[j]) + (y1[i] - y2[j]) * (y1[i] - y2[j])))
				.min().getAsDouble()).min().getAsDouble();
	}

	// BEGIN KAWIGIEDIT TESTING
	// Generated by KawigiEdit 2.1.4 (beta) modified by pivanof
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int[] p1, int[] p2, int[] p3, int[] p4, int[] p5,
			boolean hasAnswer, double p6) {
