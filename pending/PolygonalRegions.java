...
public class PolygonalRegions {
			...
			for (int i = 2; i <= v; ++i) {
				for (int j = 0; j <= v; ++j) {
					...
					for (int p = 1; p < v; ++p) {
						...
						if (prevJ >= 0) {
							nextBadFirstProbs[i][j][v] = addMod(nextBadFirstProbs[i][j][v],
									multiplyMod(badFirstProbs[i - 1][prevJ][p], goodProb));
							nextGoodFirstProbs[i][j][v] = addMod(nextGoodFirstProbs[i][j][v],
		...
		for (int i = 1; i <= N; ++i) {
			for (int j = 0; j <= N; ++j) {
				for (int k = 1; k <= N; ++k) {
					...
					result = addMod(result, multiplyMod(regionNums[i] + j + (k == N ? 0 : 1), goodFirstProbs[i][j][k]));
				}
			}
		}
	...
	int[] buildRegionNums(int N) {
		return IntStream.rangeClosed(0, N).map(n -> (n * n * n * n - 6 * n * n * n + 23 * n * n - 42 * n + 24) / 24)
				.toArray();
	}
