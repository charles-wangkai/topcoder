...
public class PulseRadar {
	...
	public int[] deduceSpeeds(int[] x1, int[] y1, int[] x2, int[] y2, int[] x3, int[] y3) {
		...
		Set<Point> thirdPoints = IntStream.range(0, n).mapToObj(i -> new Point(x3[i], y3[i]))
		...
		return IntStream.range(0, n).map(i -> computeSpeed(x1[i] - x2[solution[i]], y1[i] - y2[solution[i]])).toArray();
	}

	void search(int[] x1, int[] y1, int[] x2, int[] y2, Set<Point> thirdPoints, int[] secondIndices,
			Set<Point> currentThirds, int depth) {
		int n = x1.length;

		if (depth == n) {
			++solutionCount;
			solution = Arrays.copyOf(secondIndices, n);

			return;
		}

		for (int i = depth; i < n; ++i) {
			Point third = new Point(x2[secondIndices[i]] * 2 - x1[depth], y2[secondIndices[i]] * 2 - y1[depth]);
