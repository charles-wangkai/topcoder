public class ThreeSameLetters {
	...
	public int countStrings(int L, int S) {
		int[][][] wayNums = new int[2][S + 1][4];
		wayNums[0][0][1] = 1;

		for (int length = 0; length < L; ++length) {
			int[][][] nextWayNums = new int[2][S + 1][4];
			for (int i = 0; i < 2; ++i) {
				for (int j = 0; j <= S; ++j) {
					for (int k = 1; k < 4; ++k) {
						for (int nextJ = 1; nextJ <= S; ++nextJ) {
							...
							nextWayNums[nextI][nextJ][nextK] = addMod(nextWayNums[nextI][nextJ][nextK],
									wayNums[i][j][k]);
						}
					}
				}
		...
		for (int i = 1; i <= S; ++i) {
			for (int j = 1; j < 4; ++j) {
				result = addMod(result, wayNums[1][i][j]);
			}
		}
