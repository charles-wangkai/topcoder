...
public class BaseMystery {
	...
	public int[] getBase(String equation) {
		...
		int minBase = Math.max(2,
				Arrays.stream(parts)
						.mapToInt(part -> part.chars().map(ch -> Character.digit(ch, MAX_BASE)).max().getAsInt()).max()
						.getAsInt() + 1);

		return IntStream.rangeClosed(minBase, MAX_BASE).filter(base -> Integer.parseInt(parts[0], base)
				+ Integer.parseInt(parts[1], base) == Integer.parseInt(parts[2], base)).toArray();
	}
