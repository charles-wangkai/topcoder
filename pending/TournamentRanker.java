...
public class TournamentRanker {
	public String[] rankTeams(String[] names, String[] lostTo) {
		Map<String, String> nameToWinner = IntStream.range(0, names.length).boxed()
				.collect(Collectors.toMap(i -> names[i], i -> lostTo[i]));

		Map<String, Integer> nameToWinCount = Arrays.stream(names).collect(Collectors.toMap(name -> name, name -> 0));
		...
		List<String>[] winCountToNames = new List[nameToWinCount.values().stream().mapToInt(x -> x).max().getAsInt()
				+ 1];
		...
		for (int i = winCountToNames.length - 1; i >= 0; --i) {
			System.out.println(i + ": " + winCountToNames[i]);
			System.out.println("nameToRank: " + nameToRank);
			Collections.sort(winCountToNames[i], (name1, name2) -> Integer
					.compare(nameToRank.get(nameToWinner.get(name1)), nameToRank.get(nameToWinner.get(name2))));
	...
	public static void main(String[] args) {
		...
		// ----- test 1 -----
		p0 = new String[] { "DUKE", "SETON HALL", "ILLINOIS", "CINCINNATI", "NORTH CAROLINA", "TEXAS", "XAVIER",
				"MISSISSIPPI STATE" };
		p1 = new String[] { "", "DUKE", "DUKE", "ILLINOIS", "TEXAS", "XAVIER", "DUKE", "XAVIER" };
		p2 = new String[] { "DUKE", "XAVIER", "ILLINOIS", "TEXAS", "SETON HALL", "MISSISSIPPI STATE", "CINCINNATI",
				"NORTH CAROLINA" };
