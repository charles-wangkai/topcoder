...
public class BombSweeper {
	public double winPercentage(String[] board) {
		...
		int winNum = IntStream.range(0, row).map(r -> (int) IntStream.range(0, col)
				.filter(c -> IntStream.rangeClosed(-1, 1).allMatch(dr -> IntStream.rangeClosed(-1, 1).allMatch(dc -> {
					int adjR = r + dr;
					int adjC = c + dc;

					return adjR < 0 || adjR >= row || adjC < 0 || adjC >= col || board[adjR].charAt(adjC) == '.';
				}))).count()).sum();
		int lossNum = Arrays.stream(board).mapToInt(line -> (int) line.chars().filter(c -> c == 'B').count()).sum();
