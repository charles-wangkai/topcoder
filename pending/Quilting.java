...
public class Quilting {
	...
	String chooseColor(String[][] colors, Map<String, Integer> colorToCount, String[] colorList, int r, int c) {
		int length = colors.length;
		int width = colors[0].length;

		int adjNumForResult = Integer.MAX_VALUE;
		String result = null;
		for (String color : colorList) {
			...
			for (int dr = -1; dr <= 1; ++dr) {
				for (int dc = -1; dc <= 1; ++dc) {
					...
					if (adjR >= 0 && adjR < length && adjC >= 0 && adjC < width && colors[adjR][adjC] == color) {
						++adjNum;
					}
				}
			}

			if (adjNum < adjNumForResult || (adjNum == adjNumForResult
					&& colorToCount.getOrDefault(color, 0) < colorToCount.getOrDefault(result, 0))) {
				adjNumForResult = adjNum;
				result = color;
			}
		}
