...
public class Mailbox {
  public int impossible(String collection, String[] address) {
    ...
    return (int)
        Arrays.stream(address).filter(addr -> !canForm(collectionCharToCount, addr)).count();
  }

  boolean canForm(Map<Character, Integer> collectionCharToCount, String addr) {
    Map<Character, Integer> addrCharToCount = buildCharToCount(addr.replace(" ", ""));

    return addrCharToCount.keySet().stream()
        .allMatch(
            addrChar ->
                addrCharToCount.get(addrChar) <= collectionCharToCount.getOrDefault(addrChar, 0));
  }
