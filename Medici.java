...
public class Medici {
  public int winner(int[] fame, int[] fortune, int[] secrets) {
    int[] points =
        IntStream.range(0, fame.length)
            .map(i -> Math.min(Math.min(fame[i], fortune[i]), secrets[i]))
            .toArray();
    int maxPoint = Arrays.stream(points).max().getAsInt();
    int[] indicesWithMaxPoint =
        IntStream.range(0, points.length).filter(i -> points[i] == maxPoint).toArray();
