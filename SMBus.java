...
public class SMBus {
  public int transmitTime(String[] messages, int[] times) {
    int result = 0;
    Set<Integer> restIndices =
        IntStream.range(0, messages.length).boxed().collect(Collectors.toSet());
    while (!restIndices.isEmpty()) {
      int length = 0;
      Set<Integer> candidates = restIndices;
      while (!(candidates.size() == 1
          && length == messages[candidates.iterator().next()].length())) {
        result += candidates.stream().mapToInt(candidate -> times[candidate]).max().getAsInt();

        int length_ = length;
        int minCh =
            candidates.stream()
                .mapToInt(candidate -> messages[candidate].charAt(length_))
                .min()
                .getAsInt();
        candidates =
            candidates.stream()
                .filter(candidate -> messages[candidate].charAt(length_) == minCh)
  ...
  public static void main(String[] args) {
    ...
    p0 =
        new String[] {
          ...
          "011143279122561420",
          "001046384966198",
          "00200570375817801109530240012",
          "0003163277589822",
          "01100240744590150136673219652442108",
          "012033596872642679096489479354",
          "0121059494098",
          "00001002303019117948961792176",
          "00216399923558",
          "02014",
          ...
          "0211798433083855430",
          "00000005842153604632996228135814",
          ...
          "00110904" + "98947092002457447355036811372647896489218",
          "000001275414757476198997466",
          "00010014",
          "00111025861963467834393486017602553072649743",
          "000102085",
          "00120882661783539",
          "01100018938145727291185020",
          "01100191373790478446634214244459341793",
          "0001129060",
          "001111287431066271555393813846448",
          "011010160778408696098486370196313",
          "0002125146381515395"
        };
    p1 =
        new int[] {
          42, 86, 47, 86, 32, 95, 2, 98, 17, 58, 31, 32, 85, 77, 87, 61, 1, 20, 15, 80, 20, 95, 55,
          87, 52, 14, 55, 68, 2, 66, 67, 3, 28, 97, 100, 67, 65, 20, 28, 77, 93, 64
        };
    p2 = 71048;
