...
public class WhatSort {
	...
	public String sortType(String[] name, int[] age, int[] wt) {
		String[] candidates = Arrays.stream(ORDERS).filter(order -> {
			int[] sortedIndices = IntStream.range(0, name.length).boxed().sorted((i1, i2) -> {
			...
			return IntStream.range(0, sortedIndices.length).allMatch(i -> name[sortedIndices[i]].equals(name[i])
					&& age[sortedIndices[i]] == age[i] && wt[sortedIndices[i]] == wt[i]);
		}).toArray(String[]::new);
	...
	private static boolean KawigiEdit_RunTest(int testNum, String[] p0, int[] p1, int[] p2, boolean hasAnswer,
			String p3) {
