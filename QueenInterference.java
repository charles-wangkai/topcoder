...
public class QueenInterference {
  ...
  public int numSteps(int n) {
    ...
    while (true) {
      int[] reachableIndices =
          IntStream.range(0, n)
              .filter(
                  i ->
                      IntStream.range(0, n)
                          .anyMatch(j -> j != i && isReachable(i, queens[i], j, queens[j])))
              .toArray();
      ...
      int minReachableCount =
          IntStream.rangeClosed(1, n).map(r -> reachableCounts[r]).min().getAsInt();

      int[] rowsWithMinReachableCount =
          IntStream.rangeClosed(1, n)
              .filter(r -> reachableCounts[r] == minReachableCount)
              .toArray();
