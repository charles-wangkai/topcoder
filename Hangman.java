...
public class Hangman {
	public String guessWord(String feedback, String[] words) {
		String[] candidates = Arrays.stream(words).filter(word -> isMatch(feedback, word)).toArray(String[]::new);

		return (candidates.length == 1) ? candidates[0] : "";
	}

	boolean isMatch(String feedback, String word) {
		return feedback.length() == word.length() && IntStream.range(0, 26).allMatch(i -> {
			char letter = (char) (i + 'A');

			int[] feedbackIndices = IntStream.range(0, feedback.length()).filter(j -> feedback.charAt(j) == letter)
					.toArray();
			int[] wordIndices = IntStream.range(0, word.length()).filter(j -> word.charAt(j) == letter).toArray();
	...
	public static void main(String[] args) {
		...
		// ----- test 4 -----
		p0 = "---H-O-H-B-OPHOB--";
		p1 = new String[] { "ROSACEA", "GYROVAGUE", "SHACONIAN", "ALTITONANT", "BRACHIATION", "CERCOPITHECAN",
				"SCOLECOPHAGOUS", "RESISTENTIALISM", "SLUBBERDEGULLION", "AICHMORHABDOPHOBIA", "SPECTOCLOACAPHOBIA",
