...
public class EmptyTheBox {
	...
	double search(Map<Integer, Double> rollToProb, int code) {
		if (cache.containsKey(code)) {
			return cache.get(code);
		}

		int tokenSum = computeTokenSum(code);
		Map<Integer, Double> rollToMinPenalty = new HashMap<>();
		for (int roll : rollToProb.keySet()) {
			rollToMinPenalty.put(roll, (double) tokenSum);
		}

		for (int nextCode = 0; nextCode < code; ++nextCode) {
			if ((nextCode & code) == nextCode) {
				int nextTokenSum = computeTokenSum(nextCode);
				int roll = tokenSum - nextTokenSum;

				if (rollToProb.containsKey(roll)) {
					rollToMinPenalty.put(roll, Math.min(rollToMinPenalty.get(roll), search(rollToProb, nextCode)));
				}
			}
		}
		...
		double result = 0;
		for (int roll : rollToProb.keySet()) {
			result += rollToProb.get(roll) * rollToMinPenalty.get(roll);
		}
	...
	int computeTokenSum(int code) {
		int result = 0;
		for (int token = 1; code != 0; ++token) {
			if ((code & 1) != 0) {
				result += token;
			}
