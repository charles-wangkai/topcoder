...
public class Homomorphism {
	public int count(String u, String v) {
		...
		int count0 = (int) u.chars().filter(ch -> ch == '0').count();
		int count1 = u.length() - count0;

		if (count1 == 0) {
			if (v.length() % u.length() == 0 && IntStream.range(1, u.length())
					.allMatch(i -> v.substring(i * (v.length() / u.length()), (i + 1) * (v.length() / u.length()))
	...
	String map(String u, String value0, String value1) {
		return u.chars().mapToObj(ch -> (ch == '0') ? value0 : value1).collect(Collectors.joining());
	}

	String invert(String s) {
		return s.chars().mapToObj(ch -> String.valueOf((char) ('0' + '1' - ch))).collect(Collectors.joining());
	}
